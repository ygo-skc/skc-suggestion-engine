...
JQ_EXPRESSION='with_entries(select([.key] | inside(["MONGODB_X509", "SSL_CERTIFICATE", "SSL_CA_BUNDLE", "SSL_PRIVATE_KEY"]) | not)) | to_entries[] | "\(.key)=\"\(.value)\""'
...
# Download - Prod
doppler secrets download -p skc-suggestion-engine -c prod --no-file --format json | jq -r "$JQ_EXPRESSION" > .env_docker
