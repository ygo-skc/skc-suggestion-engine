...
func FindDesiredCardInDBUsingID(cardID string) (contract.Card, error) {
	var card contract.Card
...
func FindDesiredCardInDBUsingMultipleCardIDs(cards []string) (contract.DeckListContents, util.APIError) {
	args := make([]interface{}, len(cards))
	for index, cardId := range cards {
		args[index] = cardId
	}
	cardData := map[string]contract.Card{}
...
func FindDesiredCardInDBUsingName(cardName string) (contract.Card, error) {
	var card contract.Card
